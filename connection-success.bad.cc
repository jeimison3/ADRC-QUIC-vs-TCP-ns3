 ...
 *   - QUIC: INITIAL→OPEN success rate & latency (socket callbacks, igual ao TCP)
 *   - Background: tráfego UDP de fundo para simular ruído de rede
 *
...
static std::map<Ptr<Socket>, double> g_quicStartTimes;
static uint32_t g_quicCompleted = 0;
static uint32_t g_quicTotal = 0;
...
// ===========================================================================
// QUIC Socket Callbacks (mesmo padrão do TCP)
// ===========================================================================

static void
OnQuicConnectionSucceeded(Ptr<Socket> sock)
{
  auto it = g_quicStartTimes.find(sock);
  if (it == g_quicStartTimes.end()) return;
  ...
  r.succeeded = true;
  r.failureReason = "";
  g_quicResults.push_back(r);
  g_quicStartTimes.erase(it);
  g_quicCompleted++;

  if (g_verboseConn)
    std::cout << now << "s QUIC-OK [" << g_quicCompleted << "/" << g_quicTotal
              << "] hs=" << latMs << " ms" << std::endl;
}

static void
OnQuicConnectionFailed(Ptr<Socket> sock)
{
  auto it = g_quicStartTimes.find(sock);
  if (it == g_quicStartTimes.end()) return;
  ...
  if (g_verboseConn)
    std::cout << Simulator::Now().GetSeconds()
              << "s QUIC-FAIL: " << r.failureReason << std::endl;
}

static void
OnQuicNormalClose(Ptr<Socket> sock)
{
  auto it = g_quicStartTimes.find(sock);
  if (it != g_quicStartTimes.end())
    g_quicStartTimes.erase(it);
}

static void
OnQuicErrorClose(Ptr<Socket> sock)
{
  auto it = g_quicStartTimes.find(sock);
  if (it != g_quicStartTimes.end())
    {
...
// ===========================================================================
// Connection scheduler — QUIC (mesmo padrão do TCP)
// ===========================================================================

static void
ScheduleQuicConnection(Ptr<Node> client, Ipv4Address serverAddr, uint16_t port,
                       uint32_t remaining, Time interval, Time /*timeout*/)
{
  if (remaining == 0) return;

  Ptr<Socket> sock = Socket::CreateSocket(client, QuicSocketFactory::GetTypeId());
  sock->SetConnectCallback(
    MakeCallback(&OnQuicConnectionSucceeded),
    MakeCallback(&OnQuicConnectionFailed));
  sock->SetCloseCallbacks(
    MakeCallback(&OnQuicNormalClose),
    MakeCallback(&OnQuicErrorClose));

  double now = Simulator::Now().GetSeconds();
  g_quicStartTimes[sock] = now;
  ...
  if (ret != 0)
    {
      ...
      r.failureReason = "ImmediateConnectError";
      g_quicResults.push_back(r);
      g_quicStartTimes.erase(sock);
      g_quicCompleted++;
    }

  Simulator::Schedule(interval,
                      &ScheduleQuicConnection,
                      client, serverAddr, port,
                      remaining - 1, interval, interval);
}
...
CheckTimeouts(double timeoutLimit)
{
  ...
  for (auto it = g_quicStartTimes.begin(); it != g_quicStartTimes.end(); )
    {
      if (now - it->second > timeoutLimit)
        {
          QuicConnectionResult r;
...
WriteSummaryCsv(const std::string& path,
                ...
                uint32_t quicSuccess, uint32_t quicAttempts,
                ...
                bool enableQuic)
{
  ...
  if (enableQuic && numQuicClients > 0)
    {
      writeRow("QUIC", quicSuccess, quicAttempts,
...
WriteConnectionsCsv(const std::string& path)
{
  ...
  for (auto& r : g_quicResults)
    {
      csv << "QUIC," << r.startTime << "," << r.latencyMs << ","
          << (r.succeeded ? 1 : 0) << "," << r.failureReason << ",0" << std::endl;
    }
  csv.close();
}
...
int main(int argc, char* argv[])
{
  ...
  g_quicTotal = numQuicClients * connPerClient;
  ...
  // QUIC clients — ScheduleQuicConnection (mesmo padrão do TCP)
  if (enableQuic && numQuicClients > 0)
    {
      uint16_t quicPort = 8000;
      for (uint32_t i = 0; i < numQuicClients; i++)
        {
          Simulator::Schedule(Seconds(warmupTime + i * 0.1),
                              &ScheduleQuicConnection,
                              quicStaNodes.Get(i), serverAddr, quicPort,
                              connPerClient,
                              Seconds(connectionInterval),
                              Seconds(connectionTimeout));
        }
  ...
  uint32_t quicAttempts = g_quicTotal;
  ...
  if (enableQuic && numQuicClients > 0)
    {
      std::cout << "\n--- QUIC ---" << std::endl;
      std::cout << "  Tentativas:     " << quicAttempts << std::endl;
      std::cout << "  Sucessos:       " << quicSuccess << std::endl;
      std::cout << "  Falhas:         " << (quicAttempts - quicSuccess) << std::endl;
      std::cout << "  Taxa de sucesso:" << std::fixed << std::setprecision(1)
                << (quicAttempts > 0 ? 100.0 * quicSuccess / quicAttempts : 0) << " %" << std::endl;
